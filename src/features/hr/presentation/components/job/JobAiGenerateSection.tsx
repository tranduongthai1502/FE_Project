...
export function JobAiGenerateSection({
  ...
  const formErrors = criteriaCtrl.jobFormErrors || {}
        ...
        <form className={`${styles.jobForm} ${styles.aiJobForm}`} noValidate>
          <section className={styles.aiJobInputPanel}>
            <label className={styles.fullField}>
              ...
              <input className={formErrors.title ? styles.jobInputError : undefined} value={criteriaCtrl.jobForm.title} maxLength={jobTitleMaxLength} onChange={(e) => criteriaCtrl.updateJobFormField('title', e.target.value)} placeholder="e.g. Senior Product Designer" />
              <JobFieldError message={formErrors.title?.message} />
            </label>
            <label className={styles.aiDepartmentField}>
              <span>Department <b>*</b></span>
              <select className={formErrors.department ? styles.jobInputError : undefined} value={criteriaCtrl.jobForm.department} onChange={(e) => criteriaCtrl.updateJobFormField('department', e.target.value)}><option value="">Select department type</option><option value="Engineering">Engineering</option><option value="Design">Design</option><option value="Marketing">Marketing</option><option value="Operations">Operations</option><option value="Data">Data</option></select>
              <JobFieldError message={formErrors.department?.message} />
            </label>
            <div className={styles.aiLocationField}>
              <span>Location <b>*</b></span>
              <div className={styles.aiLocationControls}>
                <div>
                  <select className={formErrors.locationType ? styles.jobInputError : undefined} value={criteriaCtrl.jobForm.locationType} onChange={(e) => criteriaCtrl.updateJobFormField('locationType', e.target.value)}><option value="OFFICE">Office</option><option value="REMOTE">Remote</option><option value="HYBRID">Hybrid</option></select>
                  <JobFieldError message={formErrors.locationType?.message} />
                </div>
                <div>
                  <div className={styles.iconInput}>
                    ...
                    <input className={formErrors.location ? styles.jobInputError : undefined} value={criteriaCtrl.jobForm.location} maxLength={FIELD_LENGTH_LIMITS.defaultText} onChange={(e) => criteriaCtrl.updateJobFormField('location', e.target.value)} placeholder="e.g. San Francisco, CA" />
                  </div>
                  <JobFieldError message={formErrors.location?.message} />
                </div>
              </div>
            </div>
            <label>
              <span>Application Deadline</span>
              <input className={formErrors.applicationDeadline ? styles.jobInputError : undefined} type="text" value={criteriaCtrl.deadlineInputValue} maxLength={FIELD_LENGTH_LIMITS.defaultText} onChange={(e) => criteriaCtrl.updateDeadlineInputValue(e.target.value)} placeholder="dd/mm/yyyy" />
              <JobFieldError message={formErrors.applicationDeadline?.message} />
            </label>
            <div className={styles.aiSalaryField}>
              ...
              <div className={styles.aiSalaryControls}>
                <div className={styles.salaryInputSlot}>
                  <div className={`${styles.moneyInput} ${formErrors.salaryMin ? styles.moneyInputError : ''}`}><span>$</span><input aria-label="Minimum salary" type="text" inputMode="decimal" value={criteriaCtrl.salaryInputValues.salaryMin} maxLength={FIELD_LENGTH_LIMITS.defaultText} onChange={(e) => criteriaCtrl.updateSalaryField('salaryMin', e.target.value)} placeholder="0" /></div>
                  <JobFieldError message={formErrors.salaryMin?.message} />
                </div>
                <div className={styles.salaryInputSlot}>
                  <div className={`${styles.moneyInput} ${formErrors.salaryMax ? styles.moneyInputError : ''}`}><span>$</span><input aria-label="Maximum salary" type="text" inputMode="decimal" value={criteriaCtrl.salaryInputValues.salaryMax} maxLength={FIELD_LENGTH_LIMITS.defaultText} onChange={(e) => criteriaCtrl.updateSalaryField('salaryMax', e.target.value)} placeholder="0" /></div>
                  <JobFieldError message={formErrors.salaryMax?.message} />
                </div>
              </div>
            </div>
            <label className={`${styles.fullField} ${styles.aiTextAreaField}`}>
              <span>Key Skills <b>*</b></span>
              <textarea className={formErrors.requirements ? styles.jobInputError : undefined} value={criteriaCtrl.jobForm.requirements} maxLength={FIELD_LENGTH_LIMITS.jobDescription} onChange={(e) => criteriaCtrl.updateJobFormField('requirements', e.target.value)} placeholder="Add skill..." />
              <JobFieldError message={formErrors.requirements?.message} />
            </label>
